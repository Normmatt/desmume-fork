...
#elif defined(_WIN32) && !defined(WXPORT)
...
#ifdef FB_USE_RGBA
static FORCEINLINE u32 RGBA8888_32_To_RGBA6665_32Rev(const u32 srcPix)
{
...
static FORCEINLINE u32 RGBA8888_32Rev_To_RGBA6665_32Rev(const u32 srcPix)
{
	const u32 dstPix = (srcPix >> 2) & 0x3F3F3F3F;
...
static void OGLConvertFramebufferRGBA(const u32 *__restrict srcBuffer, u32 *dstBuffer)
{
	...
	// Convert from 32-bit RGBA8888 format to 32-bit RGBA6665 reversed format. OpenGL
...
static void OGLConvertFramebufferBGRA(const u32 *__restrict srcBuffer, u32 *dstBuffer)
{
	if (srcBuffer == NULL || dstBuffer == NULL)
	{
		return;
...
#ifdef FB_USE_RGBA
		glReadPixels(0, 0, 256, 192, GL_RGBA, GL_UNSIGNED_BYTE, pixBuffer);
#else
...
#ifdef FB_USE_RGBA
	OGLConvertFramebufferRGBA(pixBuffer, (u32 *)gfx3d_convertedScreen);
#else
	OGLConvertFramebufferBGRA(pixBuffer, (u32 *)gfx3d_convertedScreen);
#endif
...
static bool OGLInitShaders(const char *oglExtensionString)
{
	...
	INFO("OpenGL: Shaders aren't supported by OSMesa.\n");
#endif
...
static bool OGLInitPBOs(const char *oglExtensionString, bool vboSupport)
{
#if defined(USE_GLES) || defined(USE_GLES2)
	return false;
#else
	...
	bool isFeatureSupported = false;
#else
	bool isFeatureSupported = ( !vboSupport ||
...
static bool OGLInitVAOs(const char *oglExtensionString, bool shaderSupport, bool vboSupport)
{
#ifdef USE_GLES
	return false;
#else
	...
	bool isFeatureSupported = ( !shaderSupport ||
								!vboSupport ||
							    strstr(oglExtensionString, "GL_OES_vertex_array_object") == NULL ) ? false : true;
#elif !defined(GL_ARB_vertex_array_object) && !defined(GL_APPLE_vertex_array_object)
	bool isFeatureSupported = false;
#else
	bool isFeatureSupported = ( !shaderSupport ||
							    !vboSupport ||
...
static bool OGLInitFBOs(const char *oglExtensionString)
{
#if defined(USE_GLES)
	return false;
	bool isFeatureSupported = (strstr(oglExtensionString, "GL_OES_framebuffer_object") == NULL ||
							   strstr(oglExtensionString, "GL_OES_packed_depth_stencil") == NULL) ? false : true;
#elif defined(USE_GLES2)
	return false;
	bool isFeatureSupported = (strstr(oglExtensionString, "GL_OES_packed_depth_stencil") == NULL) ? false : true;
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 256, 192, 0, GL_RGBA, GL_UNSIGNED_SHORT_1_5_5_5_REV, NULL);
...
static bool OGLInitMultithreading(unsigned int coreCount)
{
	...
	if (coreCount > 1)
	{
#if defined(_WINDOWS) || defined(USE_GLES) || defined(USE_GLES2)
...
static bool OGLInitRenderStates(const char *oglExtensionString, bool shaderSupport)
{
...
#if defined(USE_GLES)
	bool isTexMirroredRepeatSupported = (strstr(oglExtensionString, "GL_OES_texture_mirrored_repeat") == NULL) ? false : true;
	bool isBlendFuncSeparateSupported = (strstr(oglExtensionString, "GL_OES_blend_func_separate") == NULL) ? false : true;
	bool isBlendEquationSeparateSupported = (strstr(oglExtensionString, "GL_OES_blend_equation_separate") == NULL || 
											 strstr(oglExtensionString, "GL_EXT_blend_minmax") == NULL) ? false : true;
#elif defined(USE_GLES2)
	bool isTexMirroredRepeatSupported = true;
	bool isBlendFuncSeparateSupported = true;
	bool isBlendEquationSeparateSupported = (strstr(oglExtensionString, "GL_EXT_blend_minmax") == NULL) ? false : true;
#else
	...
	INFO("OpenGL : isTexMirroredRepeatSupported : %d, isBlendFuncSeparateSupported : %d, isBlendEquationSeparateSupported : %d\n",
		(int)isTexMirroredRepeatSupported, (int)isBlendFuncSeparateSupported, (int)isBlendEquationSeparateSupported);
...
static char OGLInit(void)
{
	...
	isPBOSupported = OGLInitPBOs(oglExtensionString, isVBOSupported);
	isShaderSupported = OGLInitShaders(oglExtensionString);
	isVAOSupported = OGLInitVAOs(oglExtensionString, isShaderSupported, isVBOSupported);
	...
	INFO("OpenGL : isVBOSupported : %d, isPBOSupported : %d, isShaderSupported : %d, isVAOSupported : %d, isFBOSupported : %d\n",
		(int)isVBOSupported, (int)isPBOSupported, (int)isShaderSupported, (int)isVAOSupported, (int)isFBOSupported);

#ifdef FB_USE_RGBA
	INFO("OpenGL: FB_USE_RGBA\n");
#else
	INFO("OpenGL: FB_USE_BGRA\n");
#endif
...
static void GL_ReadFramebuffer()
{
	...
	if (isPBOSupported)
	{
		...
		glBindBufferARB(GL_PIXEL_PACK_BUFFER, pboRenderDataID[bufferIndex]);
#ifdef FB_USE_RGBA
		glReadPixels(0, 0, 256, 192, GL_RGBA, GL_UNSIGNED_BYTE, 0);
#else
