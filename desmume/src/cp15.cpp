...
BOOL armcp15_t::moveARM2CP(u32 val, u8 CRn, u8 CRm, u8 opcode1, u8 opcode2)
{
	...
	case 1:
		if((opcode1==0) && (opcode2==0) && (CRm==0))
		{
			...
			u32 intVector_old = NDS_ARM9.intVector;
			NDS_ARM9.intVector = 0xFFFF0000 * (BIT13(val));
			NDS_ARM9.LDTBit = !BIT15(val); //TBit
			if (intVector_old != NDS_ARM9.intVector)
			{
				if (arm_cpubase)
					arm_cpubase->Clear[NDS_ARM9.proc_ID](0, CPUBASE_FLUSHALL);
			}
	...
	case 7:
		...
		if((CRm==5)&&(opcode1==0))
		{
			...
			{
				if (arm_cpubase)
					arm_cpubase->Clear[NDS_ARM9.proc_ID](val, 32);
